...
import { MapPin, X, Search, Loader2, ExternalLink, Download, Map, Satellite, Layers } from 'lucide-react'
...
export default function MapsPage({ places: initialPlaces, categories }: MapsPageProps) {
    ...
    useEffect(() => {
        ...
        if (scrapingSuccess) {
            ...
            toast.success('Data Berhasil Dimuat!', {
                description: `${data.count} tempat telah ditambahkan ke peta${data.cached ? ' (dari cache)' : ''}`,
                duration: 5000,
            })
            ...
            if (data.status === 'success') {
                ...
                if (results.length > 0) {
                    toast.success('Scraping Berhasil!', {
                        description: `Ditemukan ${results.length} tempat${data.cached ? ' (dari cache)' : ''}. Halaman akan dimuat ulang...`,
                        duration: 2000,
                    })

                    // Save notification data to localStorage for post-reload
                    localStorage.setItem('scraping_success', JSON.stringify({
                    ...
                    toast.warning('Tidak Ada Hasil', {
                        description: 'Scraping berhasil tapi tidak ada hasil ditemukan. Coba kata kunci atau lokasi lain.',
                    })
                }
            } else {
